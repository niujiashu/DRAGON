...
print '##########################################################'
print 'Welcome to the lepton flux plotting routine               '
print '##########################################################'
...
norm_DM = [1.0, 1.0, 1.0]             # normalization factor for the DM component   
...
colors_DM_component = ['black', 'red', 'blue']
...
#
		elif hdulist[i].header['A'] == 0 and hdulist[i].header['Z_'] == -1 and hdulist[i].header['EXTRA'] == 0 and hdulist[i].header['SEC'] == 0 and hdulist[i].header['DM'] == 1:
			DM_found = 1
			print 'reading DM component'
			current_HDU = hdulist[i].data 
			for ie in xrange(0,dimE):
				table_DM[ie] = table_DM[ie] + current_HDU[ie]	
	...
	if (DM_found == 0):
		print "Warning: DM component not found in ", Run_Names[k]
	...
	for ie in xrange(0,dimE):
		total_positrons[ie] = total_positrons[ie] + table_pos[ie] + norm_DM[k]*table_DM[ie] + norm_extra[k]*table_extra[ie] 
	total_electrons = np.zeros(dimE)
	total_electrons_mod = np.zeros(dimE)
	for ie in xrange(0,dimE):
		total_electrons[ie] = total_electrons[ie] + norm[k]*table_pri_ele[ie] + table_sec_ele[ie] + norm_DM[k]*table_DM[ie] + norm_extra[k]*table_extra[ie] 
	DM_component = np.zeros(dimE)
	for ie in xrange(0,dimE):
		DM_component[ie] = norm_DM[k]*table_DM[ie]
	...
	lis_DM,  = plt.plot(E, (DM_component*pow(E,3)), ls='-.' , color=colors_DM_component[k],lw=2)
