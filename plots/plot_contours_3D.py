...
Run_Names = ['run_3D', 'run_3D', 'run_3D']
...
for k in range(Number_of_Runs):
	...
	for i in xrange(1,n_ext):
		...
		if hdulist[i].header['A'] == A_ and hdulist[i].header['Z_'] == Z_:
			print "this is the particle you want to plot"
...
for k in range(Number_of_Runs):	
	...
	if (current_max_x > max_x):
		max_x = current_max_x
plt.axis([0,max_x, 0.,1.5],interpolation='none')
plt.xlabel('x [kpc]',fontsize=20)
plt.ylabel('profile',fontsize=20)
plt.savefig(Output_Folder + '/' + 'radial_profile_' + str(k) + '.eps',format='eps',dpi=100)
plt.show()
...
plt.ylabel('profile',fontsize=20)
plt.savefig(Output_Folder + '/' + 'vertical_profile_' + str(k) + '.eps',format='eps',dpi=100)
plt.show()
